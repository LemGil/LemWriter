...
const StudyPanel = ({ project, section, onSectionUpdate, onResourceChange }) => {
  ...
  const saveSectionBibleRef = async (updates) => {
    ...
    const current = { references, questions, points, ...updates }
  ...
  useEffect(() => {
    ...
    projectService.getNotes(section.id).then(setSectionNotes).catch(() => {})
  }, [section?.id])
  ...
  const addNote = async () => {
    ...
    setNewNoteText('')
    await saveToResources(project?.id, 'nota_estudio', {
      title: newNoteText.slice(0, 60), content: newNoteText,
    }, onResourceChange)
  }
  ...
  const addQuestion = async () => {
    ...
    setNewQuestionText('')
    await saveToResources(project?.id, 'pregunta_estudio', {
      title: newQuestionText.slice(0, 60), content: newQuestionText,
    }, onResourceChange)
  ...
  const addPoint = async () => {
    ...
    setPoints(updated)
    setNewPointText('')
    await saveToResources(project?.id, 'punto_estudio', {
      title: newPointText.slice(0, 60), content: newPointText,
    }, onResourceChange)
  ...
  const addTheme = async () => {
    ...
    await saveToResources(project.id, 'tema_doctrinal', {
      title: newThemeTitle, content: newThemeContent || '',
    }, onResourceChange)
    setNewThemeTitle('')
    setNewThemeContent('')
  ...
  const addWord = async () => {
    ...
    await saveToResources(project.id, wordType, {
      title: newWord.word, content: newWord.meaning || '',
    ...
    <BasePanel tabs={tabs} activeTab={activeTab} onTabChange={setActiveTab} accent="study">
      {(tab) => {
        switch (tab) {
            ...
            return (
              <CharacterCard
                characters={characters.map(c => ({
                  ...
                  meaning: c.meaning || '', role: c.role || '',
                  references: c.references || '', notes: c.notes || '',
                }))}
          ...
          case 'words':
            return <BibleWordsPanel projectId={project?.id} color="brand" />
          ...
          case 'notes':
            return (
              <div>
                ...
                {sectionNotes.length === 0 && <p className="text-xs text-brand-ink-3 italic font-sans">Sin notas aún</p>}
              </div>
            )
              ...
              <div>
                ...
                <div className="flex gap-1 mb-3">
                  <input
                    ...
                    className="flex-1 text-xs border rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-brand-gold"
                  />
                  <button onClick={addQuestion} className="text-xs bg-brand-gold-pale text-brand-gold-deep px-2 py-1 rounded hover:bg-brand-gold">+</button>
                </div>
                {questions.map((q, idx) => (
                  <div key={q.id} className="flex items-start gap-2 text-xs bg-brand-gold-pale/60 rounded px-2 py-1.5 mb-1">
                    ...
                    <button onClick={() => deleteQuestion(q.id)} className="text-brand-ink-3 hover:text-red-500 shrink-0">×</button>
                  </div>
                ))}
          ...
          case 'points':
            return (
              <div>
                ...
                <div className="flex gap-1 mb-3">
                  <input
                    value={newPointText} onChange={e => setNewPointText(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && addPoint()}
                    placeholder="Nuevo punto..."
                    className="flex-1 text-xs border rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-brand-gold"
                  />
                  <button onClick={addPoint} className="text-xs bg-brand-gold-pale text-brand-gold-deep px-2 py-1 rounded hover:bg-brand-gold">+</button>
                </div>
                {points.map((p, idx) => (
                  <div key={p.id} className="flex items-start gap-2 text-xs bg-brand-gold-pale/60 rounded px-2 py-1.5 mb-1">
                    ...
                    <button onClick={() => deletePoint(p.id)} className="text-brand-ink-3 hover:text-red-500 shrink-0">×</button>
                  </div>
                ))}
            ...
            return (
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => setIsAddingTheme(!isAddingTheme)}
                ...
                {isAddingTheme && (
                  <div className="bg-brand-gold-pale/60 rounded p-2 mb-3 space-y-1.5">
                    <input value={newThemeTitle} onChange={e => setNewThemeTitle(e.target.value)}
                      placeholder="Título del tema..." className="w-full text-xs border rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-brand-gold" />
                    <textarea value={newThemeContent} onChange={e => setNewThemeContent(e.target.value)}
                      placeholder="Descripción o notas..." rows={3}
                      className="w-full text-xs border rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-brand-gold resize-none" />
                    <button onClick={addTheme} disabled={!newThemeTitle.trim()}
                  ...
                  {themes.map(t => (
                    <div key={t.id} className="bg-brand-gold-pale/60 rounded px-2 py-1.5">
                      <div className="flex items-start justify-between">
                        ...
                        <button onClick={() => deleteTheme(t.id)} className="text-brand-ink-3 hover:text-red-500 shrink-0 ml-2">×</button>
                      </div>
                      {t.content && <p className="text-xs text-brand-ink-2 mt-0.5 font-serif">{t.content}</p>}
                    </div>
                  ))}
                  {themes.length === 0 && !isAddingTheme && <p className="text-xs text-brand-ink-3 italic font-sans">Sin temas aún</p>}
                </div>
              </div>
            )
