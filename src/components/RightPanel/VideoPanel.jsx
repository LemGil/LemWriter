...
const VideoPanel = ({ section, project, onResourceChange, collapsed }) => {
  ...
  const saveSectionData = async (updates) => {
    ...
    await projectService.updateSection(section.id, {
      bible_reference: JSON.stringify({
        references: current.references, scenes: current.scenes,
        notes: current.notes, words: current.words,
      }),
    })
  }
  ...
  const addScene = async () => {
    ...
    const updated = [...scenes, {
      id: Date.now(), title: newSceneTitle, duration: newSceneDuration || '', description: '',
    }]
    setScenes(updated)
    setNewSceneTitle('')
    setNewSceneDuration('')
  ...
  const updateSceneDescription = async (id, description) => {
    const updated = scenes.map(s => s.id === id ? { ...s, description } : s)
    setScenes(updated)
    await saveSectionData({ scenes: updated })
  }
            ...
            return (
              <div>
                ...
                <div className="flex gap-1 mb-3">
                  <input value={newSceneDuration} onChange={e => setNewSceneDuration(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && addScene()}
                  ...
                  <button onClick={addScene} className="text-xs bg-brand-teal text-white px-2 py-1 rounded hover:bg-brand-teal/80 font-sans">+ Agregar</button>
                </div>
                  ...
                  <div key={scene.id} className="bg-brand-gold-pale/60 rounded px-2 py-2 mb-1.5">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        {scene.duration && <span className="text-[10px] bg-brand-teal/10 text-brand-teal px-1 rounded">{scene.duration}</span>}
                      </div>
                      <button onClick={() => removeScene(scene.id)} className="text-brand-ink-3 hover:text-red-500 text-xs shrink-0 ml-1">×</button>
                    </div>
                    <textarea
                ...
                {scenes.length === 0 && <p className="text-xs text-brand-ink-3 italic font-sans">Sin escenas aún</p>}
              </div>
            )
          ...
          case 'notes':
            return (
              <div>
                ...
                <textarea
                  value={notes}
                  onChange={e => { setNotes(e.target.value); saveSectionData({ notes: e.target.value }) }}
              ...
              <div>
                ...
                <div className="flex gap-1 mb-3">
                  ...
                  <input value={newWord} onChange={e => setNewWord(e.target.value)}
                    ...
                    placeholder="Palabra..."
                    className="flex-1 text-xs border rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-brand-gold" />
                  <button onClick={addWord} className="text-xs bg-brand-teal text-white px-2 py-1 rounded hover:bg-brand-teal/80 font-sans">+</button>
                </div>
                {words.map(w => (
